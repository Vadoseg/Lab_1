...
//Generate LEDs------------------------------------------------------------------------------------
    ...
    generate for (i = 0; i < G_LEDS; i+=1) begin : gen_led
        l2b_sv_led#(
            ...
            .G_LED_WIDTH     (1                  ) //Number of output from LEDs
        ) LED (
...
//Making a process which connect o_led(Led_top) and o_func(MUX) and making o_led equal to o_func------------------------------------   
        always @(posedge w_clk)
		  o_led <= '{default: w_func};
